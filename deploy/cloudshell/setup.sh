...
#                        "anthropic" (calls the Anthropic API directly - see
#                        "Using the Anthropic API instead of Bedrock" in this
#                        directory's README.md before choosing this; it
#                        requires ANTHROPIC_API_KEY to already be set in your
#                        shell environment, and passes it to `agentcore
#                        deploy --env` as its ONLY supported mechanism - the
#                        AgentCore CLI has no native Secrets Manager/SSM
#                        Parameter Store integration for this, so the raw key
#                        value goes on the command line for that one command).
...
#      auto-created by the toolkit. CodeBuild only builds the container
#      IMAGE here - it never calls a model and never sees any API key; model
#      credentials are injected as a runtime environment variable via
#      `agentcore deploy --env`, a separate step from the image build.
...
run() {
    if [[ "$DRY_RUN" == "1" ]]; then
        # Mask the raw Anthropic key value in dry-run output; never print secrets.
        local shown=()
        local arg
        for arg in "$@"; do
...
deploy_agent() {
    local name="$1" entrypoint="$2" provider_env="$3"
    local -a deploy_env_args
    ...
    if [[ "$MODEL_PROVIDER" == "anthropic" ]]; then
        deploy_env_args=(--env "${provider_env}=anthropic" --env "ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY")
    else
        deploy_env_args=(--env "${provider_env}=bedrock")
    fi

    log_step "Deploying $name (first deploy can take several minutes - container builds run on CodeBuild)"
    run agentcore deploy --agent "$name" --auto-update-on-conflict "${deploy_env_args[@]}"
...
deploy_agent bidwright agentcore_app.py BIDWRIGHT_MODEL_PROVIDER
deploy_agent claimclarity agentcore_app_claimclarity.py CLAIMCLARITY_MODEL_PROVIDER
