...
def render_portfolio_insights_md(
    insights: list[RecurringGapInsight], total_runs_recorded: int, window_size: int
) -> str:
    """Cross-bid recurrence report: which compliance gaps have shown up
    across multiple of this company's recent bids, not just this one.
    Always written, even before there's enough history to say anything -
    a reader should be able to tell "checked, nothing recurring yet"
    apart from "this step never ran", same discipline as
    render_decision_summary_md.

    This describes a pattern already observed in past recorded runs,
    generated by plain code from validated RecurringGapInsight objects (see
    bidwright/tools/history.py:detect_recurring_gaps) - never an LLM
    judgment call, and never a prediction about future bids.
    """
    lines = ["# Portfolio Insights: Recurring Compliance Gaps", ""]

    if total_runs_recorded < 2:
        lines.append(
            "Not enough bid history yet - this is the first bid BidWright has recorded for "
            "this company. Recurrence patterns need at least 2 recorded bids to compare, so "
            "there is nothing to report yet. Run BidWright again on future RFPs and check back "
            "here."
        )
        return "\n".join(lines) + "\n"

    lines.append(
        f"**{total_runs_recorded} bid(s) recorded.** Scanning the last {window_size} for gap "
        "requirements that recurred across bids."
    )
    lines.append("")
    lines.append(
        "_Matching is exact-text on the gap requirement description, not semantic - "
        '"insufficient bonding capacity" and "bonding capacity too low" would be counted as '
        "two different gaps, not one recurring gap. Known limitation, not a bug._"
    )
    lines.append("")

    if not insights:
        lines.append(
            f"No gap has appeared in 2 or more of the last {window_size} recorded bid(s) - "
            "nothing recurring to flag right now."
        )
        return "\n".join(lines) + "\n"

    lines.append(f"**{len(insights)} recurring gap(s) found:**")
    lines.append("")
    lines.append("| Gap | Severity | Seen in | First on | Most recently on |")
    lines.append("|---|---|---|---|---|")
    for insight in insights:
        lines.append(
            f"| {insight.requirement} | {insight.severity.value.upper()} | "
            f"{insight.occurrences} of last {insight.runs_considered} | "
            f"{insight.first_seen_rfp} | {insight.most_recent_rfp} |"
        )
