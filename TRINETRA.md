...
**Verified live**, not just built: authenticated to `demo.thingsboard.io`, confirmed the exact asset name the mapping predicted ("Ramkund and near by Ghats") exists on the real tenant, and pulled a real reading (494 people, 1.4% occupancy, LOS grade A) with the 4.5x `sites.json`-vs-ThingsBoard capacity disagreement showing up exactly as designed. Ran `trinetra monitor --ghat ramkund` for real: the agent checked crowd and river signals, noticed the river gauge device for Ramkund specifically is not provisioned on this tenant (4 of KumbhDigiTwin's 7 ghats have theirs; Ramkund isn't one of them), and reported that as a data gap rather than guessing a water level.

#### Triggering Kshetra Netra from a ThingsBoard alarm, instead of polling

KumbhDigiTwin's tenant already has alarm rules and rule chains configured (`CrowdDensityWarning`/`CrowdDensityCritical`/`CrowdCollapseRisk` on `CrowdCamera`, `RiverLevelRising`/`RiverLevelDanger` on `WaterLevelGauge`, `PanicActivated` on `PanicButton`, ...). `POST /api/webhooks/thingsboard-alarm` on Trinetra's own FastAPI backend lets one of those alarms trigger Kshetra Netra directly, instead of Trinetra polling on a schedule and possibly missing the window between checks.

**Why this points at the ECS-hosted backend and not Bedrock AgentCore directly.** Bedrock AgentCore Runtime's invoke API (`InvokeAgentRuntime`) requires an AWS-SigV4-signed request; ThingsBoard's rule-engine "REST API Call" node sends a plain HTTP POST with static headers, not a signed one, so it cannot call AgentCore's invoke endpoint on its own without an intermediary that can sign the request (e.g. a small Lambda behind API Gateway). Separately, and more to the point right now: Trinetra's AgentCore deployment path (`deploy/trinetra/`) has never actually been run against a live AWS account — see the Deploying-to-AWS section below. What is live is the ECS Express dashboard, and its FastAPI backend is what this webhook is built against.

**Security**: the endpoint fails closed. `TRINETRA_WEBHOOK_SECRET` unset means every request gets 503, not open access; set, every request needs a matching `X-Webhook-Secret` header (compared with `hmac.compare_digest`, not `==`) or gets 401. This matters specifically because the live ECS deployment is otherwise unauthenticated — without this, anyone who found the URL could trigger real (paid) agent invocations by POSTing a fake alarm.

**On the ThingsBoard side**, add a node to an existing alarm rule chain (e.g. `kumbh-crowd-safety`):
1. After the "Alarm Created" trigger, add a **transformation script node** that builds exactly this payload (don't rely on ThingsBoard's native alarm message shape, which varies by trigger path — build it explicitly):
   ```javascript
   return {msg: {originator_name: metadata.deviceName || metadata.assetName || msg.originatorName,
                 alarm_type: msg.type, severity: msg.severity}, metadata: metadata, msgType: msgType};
   ```
2. Add a **REST API Call node** pointed at `https://<your-ecs-url>/api/webhooks/thingsboard-alarm`, header `X-Webhook-Secret: <the same value as TRINETRA_WEBHOOK_SECRET>`, and connect the transform node's output to it.

The response is a fast 202 (`{"accepted": true, "ghat_id": "ramkund", ...}`) or a 200 no-op for an alarm on an entity Trinetra has no ghat for (`{"accepted": false, "reason": "..."}`) — never a timeout, since the actual monitoring runs in the background. The result gets written back onto the same ThingsBoard asset as `kshetraNetraStatus`/`kshetraNetraSummary`/`kshetraNetraRecommendedAction`/`kshetraNetraCheckedAt` SERVER_SCOPE attributes (best-effort — a write-back failure is logged, never raised into the response the webhook already sent), so it's visible on a ThingsBoard dashboard widget without leaving ThingsBoard.

**Verified live**: called the same function the webhook's background task calls, against the real tenant and a real model call. On a `CrowdDensityCritical` alarm, the agent checked live crowd (1.4% occupancy, contradicting the alarm), checked the river gauge (unavailable), noticed the contradiction, and — on its own initiative, not because it was told to — ran a lookahead simulation at an estimated arrival rate to check whether the alarm might reflect a real but temporarily-cleared surge before concluding it looked like a stale or misconfigured alarm. The resulting assessment was confirmed present on the real ThingsBoard asset's attributes afterward.

### Anukaran Netra (अनुकरण नेत्र) — pushing synthetic telemetry the other way

Kshetra Netra reads from ThingsBoard; this is the write direction. Most of KumbhDigiTwin's demo tenant sits at whatever value it was last seeded with, not a live feed — Anukaran Netra pushes a plausible, reproducible, honestly-labeled crowd/river curve onto the tenant's real ghat entities so there is something changing to actually monitor.

**The one deliberately narrow LLM role in this feature.** A single model call turns a free-text request ("buildup to today's Amrit Snan peak, with an extra surge at Someshwar Ghat") into a bounded `ScenarioDirective` — a multiplier, which ghats surge, a flood intensity, a cycle length, and a short narrative. The model never picks a number that gets written to ThingsBoard. Every actual `paxCount`/`levelM`/`trendCmPerHr` value comes from `trinetra/tools/thingsboard_seed.py`'s pure, seeded, documented tick math, reading each ghat's own real `safeCapacity`/`areaSqm`/`warningLevelM`/`dangerLevelM` attributes off ThingsBoard itself — not Trinetra's own `sites.json`, which only knows one of KumbhDigiTwin's seven provisioned ghats. `surge_asset_names` is also re-validated against the known-ghats list after the model answers, not just prompted against it, so a hallucinated ghat name is stripped rather than trusted.

**The curve is a monotonic build-up, not a rise-then-fall hump, and that was a real bug fix, not the original design.** The first version used a symmetric hump peaking at the cycle's midpoint and receding back to zero by the last tick — and the last tick is backdated to land at "now" so the pushed points read as a real time series. That meant the *latest* value ThingsBoard would report — the one any dashboard widget actually shows — read as calm after every run, even a run whose entire point was to simulate an approaching peak. Caught by reading the values back after a live run rather than trusting the push succeeded: fixed so "now" always means "the busiest point simulated so far."

**Pushing `paxCount` alone is sufficient — verified, not assumed.** KumbhDigiTwin's tenant already has calculated fields (`Ghat density`, `Ghat occupancy pct`, `Ghat LOS grade`) that derive `densityPaxPerSqm`/`occupancyPct`/`losGrade` automatically the moment `paxCount` updates on a Ghat asset. Anukaran Netra never computes those itself — duplicating that logic would risk drifting from whatever the tenant is actually configured to do.

```bash
trinetra seed-thingsboard --request "buildup to an Amrit Snan peak, surge at Someshwar Ghat" --tick-minutes 15
```

Requires `ANTHROPIC_API_KEY`/Bedrock credentials and `THINGSBOARD_USERNAME`/`PASSWORD` (or `THINGSBOARD_API_KEY`) — this is the one Trinetra command that writes to an external system rather than only reading or computing. Also exposed as AgentCore action `seed_thingsboard` in `agentcore_app_trinetra.py` (`{"action": "seed_thingsboard", "request": "..."}`) — code-complete, **not verified against an actual deployed AgentCore Runtime**: this session had no valid AWS credentials (`aws sts get-caller-identity` returns `InvalidClientTokenId`), so only the CLI path — which calls the identical `decide_scenario`/`run_seeding_cycle` functions — has been run for real.

**Verified live, and it found a second real gap.** A full run pushed 119 telemetry points across all seven ghats in ~67 seconds with no errors; reading values back afterward confirmed ThingsBoard's own calculated fields fired correctly (a pushed `paxCount` of 777 produced `densityPaxPerSqm`, `occupancyPct`, and `losGrade` matching the exact math in `calculated-fields.json`) and that the surge ghat read visibly higher than baseline ghats (Someshwar at 318% occupancy / LOS F against Ramkund's 203% / E in the same run). Checking the *river* side turned up a second honest gap: all four ghats with a provisioned `WaterLevelGauge` device — Odha Ghat, Someshwar Ghat, Lakshminarayan Ghat, Kapila Sangam Ghat — have the device, but none currently have `warningLevelM`/`dangerLevelM` attributes set on this tenant, so no level can be computed against a real threshold. This was originally a silent no-op; fixed to report it explicitly per ghat (`SeedingRunSummary.skipped_river_gauges`) rather than letting a missing push disappear with no trace, matching this repo's rule everywhere else: an unavailable signal is a labeled gap, never quietly treated as "nothing to report."
...
- **The ThingsBoard alarm webhook is verified against the same function a ThingsBoard rule chain would call, not against an actual configured rule chain.** `_run_alarm_triggered_monitoring` was run for real with a real payload and a real write-back confirmed on the live tenant's asset attributes. No ThingsBoard rule chain has actually been edited to add the REST API Call node this repo asks for — that's a one-time change to KumbhDigiTwin's existing tenant configuration, described but not made on the user's behalf. The webhook's shared-secret auth is a minimal protection, not a production auth scheme; rotate `TRINETRA_WEBHOOK_SECRET` and consider IP allowlisting the ThingsBoard instance's egress if this goes further than a demo.
- **Anukaran Netra's `seed_thingsboard` AgentCore action has never been invoked through an actual deployed AgentCore Runtime.** This session had no valid AWS credentials to deploy or invoke one; only the identical code path via `trinetra seed-thingsboard` has run for real, against the live ThingsBoard tenant. Deploying and invoking through AgentCore itself remains unverified, same status as every other AgentCore action in this file.
- **River-gauge telemetry cannot currently be seeded for any of KumbhDigiTwin's four gauge-equipped ghats.** Every `WaterLevelGauge` device exists on the live tenant, but none currently have `warningLevelM`/`dangerLevelM` attributes set (verified by querying all four directly), so `compute_river_reading` correctly declines rather than inventing a threshold — this is a live-tenant provisioning gap, not a code defect, and it is reported explicitly per run rather than silently. Setting those two attributes on each `"{ghat} :: level"` device closes it with no code change.
- **The seeding curve is one hand-picked shape (a monotonic quarter-sine build-up), not a library of scenario shapes.** It cannot currently represent "past peak and declining" as the *current* moment — only approaching or at a peak. A scenario that should read as already receding needs a second seeding run with a shorter, lower-multiplier directive, not a single call that captures the whole day.
