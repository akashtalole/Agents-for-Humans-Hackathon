"""Model provider selection for every agent in Trinetra.

Mirrors bidwright/config.py, claimclarity/config.py, and
glacierwatch/config.py deliberately - Trinetra is a fourth, independently
deployable project in this repo.
"""
...
def get_model():
    ...
    Resolution order:
      1. TRINETRA_MODEL_PROVIDER=anthropic|bedrock forces the provider.
    ...
    """
    provider = os.environ.get("TRINETRA_MODEL_PROVIDER", "").strip().lower()
    ...
    if use_anthropic:
        ...
        model_id = os.environ.get("TRINETRA_MODEL_ID", DEFAULT_ANTHROPIC_MODEL_ID)
        return AnthropicModel(
            client_args={"api_key": os.environ["ANTHROPIC_API_KEY"]},
            model_id=model_id,
            # A response cut off mid-tool-call at a low max_tokens ceiling can
            # turn into a retry loop that never succeeds - see bidwright/config.py.
            max_tokens=8192,
        )

    if use_bedrock:
        return os.environ.get("TRINETRA_MODEL_ID", DEFAULT_BEDROCK_MODEL_ID)

    return None


def create_agent(**kwargs) -> Agent:
    """Build a Strands Agent using Trinetra's configured model, unless the
...
def model_status() -> str:
    ...
    if provider == "bedrock" or (
        ...
        model_id = os.environ.get("TRINETRA_MODEL_ID", DEFAULT_BEDROCK_MODEL_ID)
